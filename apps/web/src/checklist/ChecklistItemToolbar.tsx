...
export function ChecklistItemToolbar({item, onEdit, onDelete, onUpdate}: {
  ...
  return <div className={checklistItemToolbarClassName}>
    ...
    {item.type === 'item' &&
            <IconButton icon={Note}{...iconProps} ariaLabel="Turn into headline" onClick={() => onUpdate({type: 'headline'})}/>}
    {item.type === 'headline' &&
            <IconButton icon={Note}{...iconProps} ariaLabel="Turn into item" onClick={() => onUpdate({type: 'item'})}/>}
