...
export function MondayOAuthBoundary({children}: { children: ReactElement }) {
    ...
    const signUp = useMutation(async () => {
        const redirect_uri = import.meta.env.__MONDAY_APP_HOSTING_URL__ + "/oauth/callback";
        const client_id = import.meta.env.__MONDAY_APP_CLIENT_ID__;
        const url = "https://auth.monday.com/oauth2/authorize?" + new URLSearchParams({client_id, redirect_uri}).toString();
